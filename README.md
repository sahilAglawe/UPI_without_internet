...
- **Node.js 18+** (only for frontend dev / optional manual builds) — `node -version`
- No database or Redis. Maven wrapper + npm (for UI) handle the rest.
...
└── pom.xml            Builds frontend into the JAR on package/run
```

### Option A — Single command (backend + built UI)

```cmd
mvnw.cmd spring-boot:run
```

First run downloads Maven, Node (via plugin), npm packages, and builds the React app — allow a few minutes. Then open **http://localhost:8080**.
...
Open **http://localhost:5173** (Vite proxies `/api` → port 8080).

Optional in `application.properties` for auto-open browser:
```properties
upi.mesh.browser-url=http://localhost:5173
```
...
# Dev UI: cd frontend && npm install && npm run dev
```

### Open the dashboard

- **Production-style:** http://localhost:8080 (React served from Spring Boot `static/`)
...
```cmd
mvnw.cmd test -Dskip.frontend.build=true
```
...
└── src/main/
    ...
    └── java/com/demo/upimesh/
        ...
        └── config/
            ...
            └── SpaForwardingController.java Serves React index.html at /
